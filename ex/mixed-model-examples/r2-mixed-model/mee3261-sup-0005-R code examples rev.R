...
lmer.R2 <- function(mod){
  var.fixed <- var(model.matrix(mod) %*% fixef(mod))
  var.random <- sum(sapply(VarCorr(mod), c))
  var.resid <- attr(VarCorr(mod), "sc")^2
  r2.marginal <- var.fixed / (var.fixed + var.random + var.resid)
  r2.conditional <- (var.fixed + var.random) / (var.fixed + var.random + var.resid)
  return(list(
    r2.marginal = as.vector(r2.marginal),
    r2.conditional = as.vector(r2.conditional)
  ))
}
lmer.R2(mF)
...
# Fit alternative model including fixed and all random effects
mF <- lmer(Colour ~ Treatment + Habitat + (1 | Population) + (1 | Container), family = "binomial", data = Data)
...
# Fit alternative model including fixed and all random effects
mF <- lmer(Egg ~ Treatment + Habitat + (1 | Population) + (1 | Container) + (1 | Unit), family = "poisson", data = Data)
